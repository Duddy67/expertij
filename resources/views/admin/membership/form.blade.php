                    ...
                    <div class="row mt-4">
                        @if ($field->name == 'why_expertij')
                            @if ($membership->professionalAttestation)
                                @include('themes.'.$theme.'.partials.membership.edit.document-file-button', ['fileUrl' => $membership->professionalAttestation->getUrl(), 'fileName' => $membership->professionalAttestation->file_name, 'label' => 'attestation'])
                            @else
                                @include('themes.'.$theme.'.partials.membership.edit.missing-document-button', ['label' => 'attestation'])
                            @endif

                            @if ($membership->resume)
                                @include('themes.'.$theme.'.partials.membership.edit.document-file-button', ['fileUrl' => $membership->resume->getUrl(), 'fileName' => $membership->resume->file_name, 'label' => 'resume'])
                            @else
                                @include('themes.'.$theme.'.partials.membership.edit.missing-document-button', ['label' => 'resume'])
                            @endif
                        @endif
                    </div>
