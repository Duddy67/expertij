    ...
    <div class="tab-content">
            ...
            <form action="#" method="post" enctype="multipart/form-data" id="licenceForm" role="form" class="php-email-form">
                ...
                <div class="form-group" id="licence-container" data-latest-index="{{ $membership->licences->count() - 1 }}">
                    @foreach ($membership->licences as $i => $licence)
                        @include('themes.basic.partials.membership.edit.licence', ['licence' => $licence, 'i' => $i])
                    @endforeach
                ...
                @if (!$membership->associated_member)
                    <div class="row">
                        ...
                        @if ($membership->professionalAttestation)
                            @include('themes.basic.partials.membership.edit.document-file-button', ['fileUrl' => $membership->professionalAttestation->getUrl(), 'fileName' => $membership->professionalAttestation->file_name])
                        @else
                            @include('themes.basic.partials.membership.edit.missing-document-button')
                        @endif
                        </div>
                        <div class="col-md-6 form-group" id="resume-file-button">
                        @if ($membership->resume)
                            @include('themes.basic.partials.membership.edit.document-file-button', ['fileUrl' => $membership->resume->getUrl(), 'fileName' => $membership->resume->file_name])
                        @else
                            @include('themes.basic.partials.membership.edit.missing-document-button')
                        @endif
                        </div>
        ...
        @if ((in_array($membership->status, ['pending_subscription', 'pending_renewal']) && !$membership->hasPendingPayment()) || ($membership->status == 'member' && !$membership->hasInsurance()))
            <div class="tab-pane" id="payment">
                @include('themes.basic.partials.membership.edit.payment')
            </div>
        @endif

        @if (!empty($invoices))
            <div class="tab-pane" id="documents">
                @include('themes.basic.partials.membership.edit.document')
            </div>
        @endif
    </div>
</div>
