<div class="row border border-success rounded p-3 m-3" id="attestation-{{ $i }}-{{ $j }}">
    <div class="row">
        ...
        <div class="col-md-6 form-group">
            ...
            <input type="text" name="licences[{{ $i }}][attestations][{{ $j }}][expiry_date]" class="form-control date" data-options="['startEmpty']" id="licences.{{ $i }}.attestations.{{ $j }}.expiry_date" data-date="{{ $attestation->expiry_date->toDateString() }}" data-format="d M Y" readonly>
    ...
    <div class="row" id="attestation-file-button-{{ $i }}-{{ $j }}">
        @include('themes.basic.partials.membership.edit.document-file-button', ['fileUrl' => $attestation->document->getUrl(), 'fileName' => $attestation->document->file_name])
    </div>
    ...
    <div class="form-group" id="skill-container-{{ $i }}-{{ $j }}" data-latest-index="{{ $attestation->skills->count() - 1 }}">
        @foreach ($attestation->skills as $k => $skill)
            @include('themes.basic.partials.membership.edit.skill', ['skill' => $skill, 'i' => $i, 'j' => $j, 'k' => $k])
        @endforeach
    </div> <!-- skill container -->
