<div class="row border border-success rounded p-3 m-3" id="attestation-{{ $i }}-{{ $j }}">
    <div class="row">
        ...
        <div class="col-md-6 form-group">
            ...
            <input type="text" name="licences[{{ $i }}][attestations][{{ $j }}][expiry_date]" class="form-control date" data-options="['startEmpty']" id="licences.{{ $i }}.attestations.{{ $j }}.expiry_date" data-date="0" data-format="d M Y" readonly>
    ...
    <div class="form-group" id="skill-container-{{ $i }}-{{ $j }}" data-latest-index="{{ $k }}">
        @include('themes.basic.partials.membership.registration.skill')
    </div> <!-- skill container -->
